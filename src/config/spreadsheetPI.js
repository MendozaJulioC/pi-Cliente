...
let googleId_pi = process.env.ikeypass2;

async function getGoogleSheetPI(){
    const documento = new GoogleSpreadsheet(googleId_pi);
    await documento.useServiceAccountAuth(keys);
    await documento.loadInfo();
    const sheet     = documento.sheetsByIndex[3];
    const reports   = await sheet.getRows();
    //console.log('spreadsheetPI ',reports); 
    //console.log(reports[0].lider); 
    return reports;
}

async function getGoogleSheetPI_Lineas(){
    const documento = new GoogleSpreadsheet(googleId_pi);
    await documento.useServiceAccountAuth(keys);
    await documento.loadInfo();
    const sheet     = documento.sheetsByIndex[4];
    const reports   = await sheet.getRows();
    //console.log('spreadsheetPI_Lineas ',reports); 
    //console.log(reports[0].lider); 
    return reports;
}
...
*/
module.exports = {
    getGoogleSheetPI          : getGoogleSheetPI,
    getGoogleSheetPI_Lineas: getGoogleSheetPI_Lineas
    /*
