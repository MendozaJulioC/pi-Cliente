...
async function semaforo_inicial(){
  fetch(`http://localhost:7001/pi/api/semaforo-corte/total`)
  .then(res=> res.json())
  .then(response=>{
...
async function avance_linea_dep(){
  try {
    let info=[];
    fetch('http://localhost:7001/dep/api/dependencias/avance')
    .then(res=>res.json())
    .then(datos=>{
...
async function alertasGraph(){
  try {
    ...
    fetch(`http://localhost:7001/pi/api/semaforo-corte/alertas`)
    .then(res=> res.json())
    .then(datos=>{
    let tam = datos.data.length;
...
async function otragrafica(data){
  ...
  //console.log(cod_dep);
  try {
    fetch(` http://localhost:7001/pi/api/semaforo-corte/contador/dependencias/${cod_dep}`)
    .then(res=> res.json())
    .then(response=> {
...
async function alerta_rojos(){
  try {
    ...
    let valoresRojos=[]
    let avance=""
    fetch(`http://localhost:7001/pi/api/semaforo-corte/rojos`)
    .then(res=> res.json())
    .then(response=>{
