...
async function linea(linea){
    try {
        spinner()
        let tabla=''
        fetch(`http://api.avanzamedellin.info/pi/api/line/${linea}`)
        .then(res=>res.json())
        .then(datos=>{
...
async function componente(componente){
    try {
        ...
        let tabla=''
        fetch(`http://api.avanzamedellin.info/pi/api/componentes/${componente}`)
        .then(res=>res.json())
        .then(datos=>{
            let tam = datos.data.length;
            document.getElementById('tabla_indicadores').innerHTML="";
...
async function prg(programa){
    try {
       // spinner()
        let tabla=''
        fetch(`http://api.avanzamedellin.info/pi/api/programas/${programa}`)
        .then(res=>res.json())
        .then(datos=>{
            let tam = datos.data.length;
            document.getElementById('tabla_indicadores').innerHTML="";
...
async function _codigoIndicador(cod_indicador){
        ...
        try {
            let tabla='', tabla2='';
            fetch(`http://api.avanzamedellin.info/pi/api/indicador/${cod_indicador}`)
            .then(res => res.json())
            .then(datos => {
