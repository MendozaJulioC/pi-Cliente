...
async function _getBuscaNombreComponente(){
    ...
    try {
        fetch(`http://localhost:7001/pi/api/componentes/consulta/nombre/${nom_Componente}`)
        .then(res=> res.json())
        .then(datos=>{
          if(datos.data.length>0){
           ...
           // _graphCumplimientoComponente(avancecomponente)
            alerta_nomcomponente(nom_Componente)
             ...
             fetch(`http://localhost:7001/pi/api/componentes/consulta-programas/nombre/${nom_Componente}`)
             .then(res=> res.json())
             .then(response=>{
             ...
             fetch(`http://localhost:7001/pi/api/componentes/responsables/nombre/${nom_Componente}`)
            .then(res=> res.json())
            .then(response =>{
...
async function _getBuscaCodigoComponente(){
    ...
    if (cod_componente.length>=3){
        try {
          fetch(`http://localhost:7001/pi/api/componentes/consulta/codigo/${cod_componente}`)
            .then(res=> res.json())
            .then(datos => {
              if(datos.data.length>0){
                document.getElementById('nom_componente').innerHTML= datos.data[0].nom_componente
              ...
              // _graphCumplimientoComponente(avancecomponente)
               ...
               _graphAvanceComponente(avancecomponente)
               fetch(`http://localhost:7001/pi/api/componentes/consulta-programas/codigo/${cod_componente}`)
               .then(res=> res.json())
               .then(response=>{
                  let tabla=''
                  let tam = response.data.length;
                  ...
                  //responsables
                     fetch(`http://localhost:7001/pi/api/componentes/responsables/codigo/${cod_componente}`)
                     .then(res=> res.json())
                     .then(response3=>{
                        let tabla3=''
...
async function alerta_nomcomponente(nom_Componente){
  try {
    fetch(`http://localhost:7001/pi/api/componentes/semaforo-corte/alerta/nombre/${nom_Componente}`)
    .then(res=> res.json())
    .then(response=>{
        const dataSource = {
        chart: {
...
async function pptoComponente(cod_componente){
  try {
        fetch(`http://localhost:7001/pi/api/componente/ppto/${cod_componente}`)
        .then(res=> res.json())
        .then(datos=>{
          const dataSource = {
            chart: {
