...
async function detalleAvance(){
  try { var fecha=0; var mes=0
        ...
        //fetch(`/pi/google`)
        fetch(`http://localhost:7001/pi/api/generalpi`)
        .then( res=> res.json())
        .then(response=>{
...
async function detalleAvanceLinea(){
  try {
   ...
   // let AvanceOct2021=[]; let CumplimientoOct21=[];
    ...
    fetch(`http://localhost:7001/pi/api/genralpilineas`)
    // fetch(`/pi/google/lineas`)
    .then( res=> res.json())
    .then(response=>{
      // console.log(response.data);
        fetch(`http://localhost:7001/pi/api/corteslineas`).then(res=> res.json())
        .then(datos=>{        
...
async function triadaInicial2(datos){
  ...
  fetch(`http://localhost:7001/pa/semaforo-corte/${mes}`)
  .then(res=>res.json())
  .then(response=>{
