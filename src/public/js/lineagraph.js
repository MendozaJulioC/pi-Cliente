...
async function _buscaAvancelinea(linea){
  try {
    let tabla=''
   let avance_Dep_Line=[];
    fetch(`http://localhost:7001/pi/api/avance/line/${linea}`)
    .then(res=>res.json())
    .then(datos=>{
      _graphAvanceIndicador(datos.data[0].sum)
      fetch(`http://localhost:7001/pi/api/responsables/line/${linea}`)
      .then(res=>res.json())
      .then(response=>{
...
async function _graphCumplimientoPptal(linea){
  try {
    fetch(`http://localhost:7001/pi/api/line/financiera/${linea}`)
    .then(res=>res.json())
    .then(datos=>{
      const dataSource = {
        chart: {
...
async function componentelinea(linea){
  try {
    let avance_Comp=[]; 
    let tabla='';
    fetch('http://localhost:7001/pi/api/componentes/avance/line/'+linea)
    .then(res=>res.json())
    .then(datos=>{
...
async function programalinea(linea){
  try {
    let avance_Prg =[];
    let tabla_prg='';
    fetch(`http://localhost:7001/pi/api/programas/avance/line/${linea}`)
    .then(res=> res.json())
    .then(datos=>{
...
async function _tbl_Indicadores(linea)
{
  try {
    let tabla3 ='';
    fetch(`http://localhost:7001/pi/api/line/indicadores/resumen/${linea}`)
    .then(res=>res.json())
    .then(response=>{
      let back_semafav='';
      let tam = response.data.length;
 ...
 async function alerta_linea(linea){
  try {
    fetch(`http://localhost:7001/pi/api/line/semafav/${linea}`)
    .then(res=>res.json())
    .then(datos=>{
      const dataSource = {
        chart: {
